    ...
    return (
        <section>
            ...
            {active && <motion.div onClick={() => setActive(null)}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ duration: 0.4 }} className='fixed top-0 left-0 w-full h-full bg-black/30 bg-opacity-50 backdrop-blur-lg flex items-center justify-center z-50 cursor-pointer'>
                <motion.div layoutId={`component-${active.title}`} className='bg-white w-80 h-auto border border-transparent shadow-black/10 ring-1 ring-black/5 p-4 rounded-lg shadow-lg'>
                    <img src={`${active.image}?w=600&q=75&auto=format`}
                        alt={active.title}
                        loading="eager"
                        decoding="async"
                        width={600}
                        height={400} className='w-full h-48 object-cover rounded-md' />
                    <motion.h1 layoutId={`component-title-${active.title}`} className='text-2xl font-bold text-neutral-700 mt-4'>{active.title}</motion.h1>
                    <motion.p layoutId={`component-desc-${active.title}`} className='text-sm text-neutral-500 mt-2'>{active.description}</motion.p>
                    {
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
                {
                    items.map((item) => {
                        return <motion.div layoutId={`component-${item.title}`} key={item.id} className='flex flex-col justify-center bg-gray-300 text-black px-3 py-2 rounded-lg shadow-2xl' onClick={() => setActive(item)}>
                            <img src={`${item.image}?w=400&q=70&auto=format`}
                                alt={item.title}
                                loading="lazy"
                                decoding="async"
                                width={400}
                                height={300}
                                className="h-50 w-full rounded-lg hover:scale-105 transition-transform duration-200 object-cover" />
                            <motion.h1 layoutId={`component-title-${item.title}`} className='text-lg font-medium text-neutral-600 mt-2 text-shadow-sm '>{item.title}</motion.h1>
                            <motion.p layoutId={`component-desc-${item.title}`} className='text-sm text-neutral-500 mt-1'>{item.description}</motion.p>
                        </motion.div>
                    })
                }
            </div>
